...
- 현재 Phase 2 코드는 로컬 구현, AWS 콘솔 리소스 준비, 개발기 `public-api` Lambda 배포, Amplify 개발기 프론트 배포까지 완료된 상태다.
- GitHub Actions OIDC 기반 `public-api` Lambda 수동 배포 workflow가 1회 성공했다.
- Amplify 개발기 URL은 `https://main.dt3yir3oksp8j.amplifyapp.com`이다.
- API Gateway CORS에 Amplify 개발기 origin을 추가했고, 공개 API와 보호 API가 개발기에서 `200` 응답을 반환함을 확인했다.

## 완료된 후속 작업

- [x] 개발기 Lambda에 최신 `public-api` 바이너리 배포
- [x] Lambda CI/CD workflow 구성
- [x] GitHub Actions용 AWS OIDC IAM role 생성
- [x] GitHub secret `AWS_GITHUB_ACTIONS_ROLE_ARN` 등록
- [x] GitHub Actions에서 `Deploy public-api Lambda` workflow 1회 수동 실행
- [x] Amplify 개발기 프론트 배포
- [x] Amplify 환경변수 구성
  - `VITE_API_BASE_URL`
...
- [x] Cognito App client callback/logout URL에 Amplify 개발기 URL 추가
- [x] API Gateway CORS origin에 Amplify 개발기 URL 추가
- [x] 개발기 end-to-end 검증
  - [x] `GET /problems`
